...
module.exports = eval("require")("./limitTags");
...
/* harmony import */ var _limitTags__WEBPACK_IMPORTED_MODULE_2__ = __nccwpck_require__(237);




let repoUrl, image, regex, versionLimit;
try {
  repoUrl = _actions_core__WEBPACK_IMPORTED_MODULE_1__.getInput("repo-url");
  image = _actions_core__WEBPACK_IMPORTED_MODULE_1__.getInput("image");
  regex = new RegExp(_actions_core__WEBPACK_IMPORTED_MODULE_1__.getInput("tag-regex"));
  versionLimit = _actions_core__WEBPACK_IMPORTED_MODULE_1__.getInput("version-limit");
} catch (err) {
  const msg = "Failed to initialize action: " + err.message;
  _actions_core__WEBPACK_IMPORTED_MODULE_1__.setFailed(msg);
  console.error(msg);
  console.error(err);
  process.exit(1);
}
...
node_fetch__WEBPACK_IMPORTED_MODULE_0__(`${repoUrl}/v2/${image}/tags/list`)
.then(response => processResponse(response))
.then(json => {
  let { tags } = json;
  let filteredTags = tags.filter(tag => regex.test(tag));
  let limitedTags = _limitTags__WEBPACK_IMPORTED_MODULE_2__(filteredTags, versionLimit);
  _actions_core__WEBPACK_IMPORTED_MODULE_1__.setOutput("tags", limitedTags);
})
.catch(err => {
